...
struct OnboardingFlow: View {
    ...
    @State private var loadedChunks: [AyahChunk] = []
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                case .surahSelection:
                    SurahSelectionView(
                        onSelect: { surah in
                            ...
                            // Load chunks for level check
                            Task {
                                await loadChunksForLevelCheck(surahId: surah.id)
                            }
                        },
                ...
                case .levelCheck:
                    if let surah = selectedSurah, !loadedChunks.isEmpty {
                        LevelCheckView(
                            surah: surah,
                            chunks: loadedChunks,
                            onComplete: { level in
                                ...
                                if let childId = appState.currentChild?.id {
                                    Task {
                                        try? await supabaseService.updateChildLevel(
                                            childId: childId,
    ...
    private func loadChunksForLevelCheck(surahId: Int) async {
        do {
            loadedChunks = try await supabaseService.fetchChunks(surahId: surahId)
            if loadedChunks.isEmpty {
                // Skip level check if no chunks available
                appState.currentRoute = .home
            } else {
                currentStep = .levelCheck
            }
        } catch {
            print("Error loading chunks for level check: \(error)")
            // Skip level check on error
            appState.currentRoute = .home
        }
    }
}
